import { ref, unref, computed, reactive, onServerPrefetch } from 'vue';
import { d as defineStore, a as useNuxtApp, c as createError } from './server.mjs';
...
const useGallery = defineStore("gallery", () => {
  let slider = ref([]);
  let sections = ref([]);
  let gallery = ref({});
  let art = ref({});
  async function fetchSlider() {
    const { data } = await useFetch("/api/slider", "$7Yg2RhqpuQ");
    slider.value = data.value;
  }
  async function fetchSections() {
    const { data } = await useFetch("/api/gallery-params", "$TwccmP6bsi");
    sections.value = data.value.sections;
  }
  async function fetchGallery() {
    const { data } = await useFetch("/api/gallery", "$4rhYycfnc9");
    gallery.value = data.value;
  }
  async function fetchArt(routeParams) {
    const { data } = await useFetch("/api/gallery", "$hdZx2LOVmw");
...
//# sourceMappingURL=useGallery.486400a1.mjs.map
